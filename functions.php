...
function jrm_record_inquiry ($Like, $inqdate) {
  ...
  // Not crawler and not admin - proceed with recording
 ...
 *
 * Detects whether the current visitor is a web crawler using:
 *   1. User-Agent string matching (primary signal)
 *   2. Missing or bot-like HTTP headers
 *   3. JavaScript challenge (client-side companion)
 *
 * Usage:
 *   $detector = new CrawlerDetector();
 *   if ($detector->isCrawler()) {
 *       // serve bot-friendly content, skip personalisation, log, etc.
 *   }
 */
...
{
    /**
     * Known crawler / bot User-Agent patterns (case-insensitive).
     * Covers all major search engines, monitoring tools, and headless browsers.
     */
    private const BOT_PATTERNS = [
        ...
        'seokicks', 'seoscanners', 'linkdexbot', 'blexbot',
        ...
        // Generic
        'bot', 'crawler', 'spider', 'crawling', 'fetch', 'http-client',
        'preview', 'lighthouse', 'pagespeed',
    ];

    /**
     * HTTP headers that real browsers always send.
     * Crawlers frequently omit several of these.
     */
    private const EXPECTED_BROWSER_HEADERS = [
        'HTTP_ACCEPT',
        'HTTP_ACCEPT_LANGUAGE',
        'HTTP_ACCEPT_ENCODING',
    ];

    /** Cached result so detection only runs once per request. */
    ...
    /**
     * Returns true if the visitor is (likely) a crawler.
     */
    public function isCrawler(): bool
    {
        ...
        $this->result =
            $this->hasNoBrowser()          // empty or missing UA
            || $this->matchesBotPattern()  // known bot UA substring
            || $this->lacksHumanHeaders(); // missing mandatory browser headers

        return $this->result;
    }

    /**
     * Returns the raw User-Agent string (empty string if absent).
     */
    ...
    /**
     * Returns a human-readable reason for the classification.
     */
    public function getReason(): string
    {
        if (!$this->isCrawler()) {
            return 'Appears to be a human visitor.';
        }
        if ($this->hasNoBrowser()) {
            return 'No User-Agent header present.';
        }
        if ($this->matchesBotPattern()) {
            return 'User-Agent matches a known bot pattern.';
        }
        return 'Missing expected browser request headers.';
    }
    ...
    private function hasNoBrowser(): bool
    {
        return $this->getUserAgent() === '';
    }

    private function matchesBotPattern(): bool
    {
    ...
    private function lacksHumanHeaders(): bool
    {
        $missing = 0;
        ...
        // Flag if two or more expected headers are absent
        return $missing >= 2;
    }
}
...
// =============================================================================
// WordPress integration helper (optional — remove if not using WordPress)
// =============================================================================

/**
 * Returns true if the current WordPress visitor is a crawler.
 * Also respects WordPress's own is_feed() / WP_Http class detection.
 *
 * Add to functions.php or a must-use plugin.
 */
function wp_is_crawler(): bool
{
    // WordPress has its own lightweight UA check
    if (function_exists('wp_is_mobile') && defined('ABSPATH')) {
        // Use our richer detector, but also check WP's internal flag
        $detector = new CrawlerDetector();
...
if ($detector->isCrawler()) {
    ...
    //   - Return a lightweight HTML page without heavy JS
    //   - Skip setting personalisation cookies
    //   - Log to a separate analytics bucket
    //   - Serve pre-rendered static content

    error_log('[CrawlerDetector] Bot detected. UA: ' . $detector->getUserAgent());
    // header('X-Robots-Tag: noindex');  // uncomment to suppress indexing
} else {
    // Normal human visitor — proceed as usual
}

/*
 * Optionally output the JS challenge snippet so the browser can confirm it is
 * JavaScript-capable. See crawler-detection.js for the companion script.
 */
/**--------------------End CrawlerDetector Class----------------------------------- */
