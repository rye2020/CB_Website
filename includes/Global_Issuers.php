<option value='Aareal Bank'>Aareal Bank</option>
<option value='Australia & New Zealand Banking Group'>Australia & New Zealand Banking Group</option>
<option value='BNP Paribas Home Loan'>BNP Paribas Home Loan</option>
<option value='Bank of Montreal'>Bank of Montreal</option>
<option value='Bank of Nova Scotia'>Bank of Nova Scotia</option>
<option value='Barclays Bank PLC'>Barclays Bank PLC</option>
<option value='Bawag Bank'>Bawag Bank</option>
<option value='Bayerische Landesbank'>Bayerische Landesbank</option>
<option value='Canadian Imperial Bank of Commerce'>Canadian Imperial Bank of Commerce</option>
<option value='Cie Financement Foncier'>Cie Financement Foncier</option>
<option value='Commonwealth Bank of Australia'>Commonwealth Bank of Australia</option>
<option value='Credit Agricole Home Loan SFH'>Credit Agricole Home Loan SFH</option>
<option value='Credit Mutuel'>Credit Mutuel</option>
<option value='Credit Suisse Guernsey'>Credit Suisse Guernsey</option>
<option value='DBS Bank Ltd'>DBS Bank Ltd</option>
<option value='Deutsche Pfandbriefbank'>Deutsche Pfandbriefbank</option>							
<option value='DNB Nor Boligkreditt AS'>DNB Nor Boligkreditt AS</option>
<option value='DnB Boligkreditt'>DnB Boligkreditt</option>
<option value='Equitable Bank Canada'>Equitable Bank Canada</option>
<option value='Fédération des caisses Desjardins du Quebec'>Fédération des caisses Desjardins du Quebec</option>
<option value='Helaba'>Helaba</option>
<option value='HSBC Bank Canada'>HSBC Bank Canada</option>					  
<option value='HSBC Bank PLC'>HSBC Bank PLC</option>
<option value='ING Bank NV'>ING Bank NV</option>
<option value='Kookmin bank'>Kookmin Bank</option>
<option value='Korea Housing Finance Co'>Korea Housing Finance Co</option>
<option value='Laurentian Bank of Canada'>Laurentian Bank of Canada</option>
<option value='LBBW'>LBBW</option>
<option value='Lloyds Bank plc'>Lloyds Bank plc</option>
<option value='Muenchener Hypothekenbank eG'>Muenchener Hypothekenbank eG</option>
<option value='National Australia Bank'>National Australia Bank</option>
<option value='National Bank of Canada'>National Bank of Canada</option>
<option value='Nationwide Building Society'>Nationwide Building Society</option>							
<option value='Nord/LB'>Nord/LB</option>
<option value='Nord/LB CBB'>Nord/LB CBB</option>
<option value='Nordea Eiendomskreditt AS'>Nordea Eiendomskreditt AS</option>
<option value='Royal Bank of Canada'>Royal Bank of Canada</option>
<option value='Santander UK'>Santander UK</option>
<option value='SEB'>SEB</option>
<option value='Shinhan Bank'>Shinhan Bank</option>
<option value='Sparebanken 1 Boligkreditt'>Sparebanken 1 Boligkreditt</option>
<option value='SR Boligkreditt'>SR Boligkreditt</option>
<option value='Stadshypotek AB'>Stadshypotek AB</option>
<option value='Sumitomo Mitsui Banking Corporation'>Sumitomo Mitsui Banking Corporation</option>
<option value='Swedbank Hypotek AB'>Swedbank Hypotek AB</option>
<option value='Toronto-Dominion Bank'>Toronto-Dominion Bank</option>
<option value='UBS AG London'>UBS AG London</option>
<option value='United Overseas Bank'>United Overseas Bank</option>
<option value='Westpac Banking Corp'>Westpac Banking Corp</option>
</select>
<br />




